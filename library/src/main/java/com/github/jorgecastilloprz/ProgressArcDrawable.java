...
import static com.github.jorgecastilloprz.AnimationUtils.getAnimatedFraction;
...
final class ProgressArcDrawable extends Drawable implements Animatable {
    ...
    ProgressArcDrawable(float strokeWidth, int arcColor, boolean roundedStroke,
                        ProgressArcAnimationDuration progressArcAnimationDuration) {
        this.strokeWidth = strokeWidth;
        this.arcColor = arcColor;
        mProgressArcAnimationDuration = progressArcAnimationDuration;
    ...
    private void setupRotateAnimation() {
        rotateAnim = ArcAnimationFactory.getRotateArcAnimation(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float angle = getAnimatedFraction(animation) * 360f;
                updateCurrentRotationAngle(angle);
            }
        }, mProgressArcAnimationDuration.getRotationArcDuration());
    }

    private void setupGrowAnimation() {
        growAnim = ArcAnimationFactory.getGrowArcAnimation(
                ...
                }, mProgressArcAnimationDuration.getGrowArcDuration());
    }

    private void setupShrinkAnimation() {
        shrinkAnim = ArcAnimationFactory.getShrinkArcAnimation(
                new ValueAnimator.AnimatorUpdateListener() {
                    @Override
                    public void onAnimationUpdate(ValueAnimator animation) {
                        float animatedFraction = getAnimatedFraction(animation);
                ...
                mProgressArcAnimationDuration.getShrinkArcDuration());
    }

    private void setupCompleteAnimation() {
        completeAnim = ArcAnimationFactory.getCompleteAnimation(
                new ValueAnimator.AnimatorUpdateListener() {
                    ...
                    public void onAnimationStart(Animator animation) {
                        ...
                        int duration = mProgressArcAnimationDuration.getCompleteArcDuration() * 6;
                ...
                mProgressArcAnimationDuration.getCompleteArcDuration());
    }
